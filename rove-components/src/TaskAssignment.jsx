...
const AssignTaskModal = ({ open, onClose, members, currentUser }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      setIsWeekly(false); setIsMonthly(false); setCategory('General');
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full bg-[#121214] border border-white/10 rounded-xl px-4 py-3
                           text-white text-sm focus:outline-none focus:border-[#3B82F6] focus:ring-1 focus:ring-[#3B82F6]/40
                           transition-colors"
              >
...
export default function TaskAssignment({ familyId }) {
    ...
    if (lastDate && lastDate !== todayStr && lastDate !== yesterdayStr) {
      ...
      const resetStreak = async () => {
        try {
          ...
          console.warn('[TaskAssignment] Firestore streak reset failed (likely permission rule). Local state is reset.', err);
        }
      };
      resetStreak();
    }
    ...
    try {
      ...
      if (task.assignedTo === currentUser?.uid) {
        ...
        try {
          ...
          console.warn('[TaskAssignment] Firestore root user attributes write failed/blocked:', uErr);
        }

        try {
          await updateDoc(doc(db, 'families', familyId, 'members', currentUser.uid), updates);
        } catch (dbErr) {
          console.warn('[TaskAssignment] Firestore profile write blocked (likely permission rule). Updating local state instead:', dbErr);
        }
      }
    } catch (err) {
    ...
    try {
      ...
      if (task.assignedTo === currentUser?.uid) {
        ...
        try {
          ...
          console.warn('[TaskAssignment] Firestore profile write blocked:', dbErr);
        }
      }
    } catch (err) {
      console.error('[TaskAssignment] Error updating weeklyDone/RPG stats:', err);
    }
  ...
  const handleMonthlyToggle = useCallback(async (task, e) => {
    ...
    try {
      ...
      console.error('[TaskAssignment] Error updating monthlyDone/RPG stats:', err);
    }
  }, [familyId, currentUser, rpgStats]);
  ...
  // ── Early returns for Auth (declared after hooks to respect React rules) ──
  if (authLoading) {
    return (
